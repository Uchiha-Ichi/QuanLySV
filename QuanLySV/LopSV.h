...
public:
	...
	void xuatTrangDSLH(int tongSoDong, thaoTac& hD, int& t, char* s, int& on, thaoTac& hDSV);
	void nhapLH();
...
void DSLH::xuatTrangDSLH(int tongSoDong, thaoTac& hD, int& t, char* s, int& on, thaoTac& hDSV) {
	int x = -1, y = -1;
	int batDau = 0;
...
void DSLH::menuSetLop(int& on) {
	...
	while (ok) {
		...
		case XUAT: {
			...
			/*outtextxy(183 + 5, (169 + 205 - textheight(s)) / 2, s);*/
			xuatTrangDSLH(tongSoDong, hD, thaoTacRoi, s, on, hDSV);
			break;
		}
				 //case LOC: {
				 //	cleardevice();
